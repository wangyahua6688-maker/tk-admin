...
type SystemMessageService struct {
	dao *dao.SystemMessageDao
}

func NewSystemMessageService(d *dao.SystemMessageDao) *SystemMessageService {
...
func (s *SystemMessageService) PushToUsers(
	...
	msgLevel := normalizeMessageLevel(level)
...
func (s *SystemMessageService) ListUserIDsByPermissionIDs(ctx context.Context, permissionIDs []uint) ([]uint, error) {
	roleIDs, err := s.dao.FindRoleIDsByPermissionIDs(ctx, uniquePositiveIDs(permissionIDs))
	if err != nil {
		return nil, err
	}
