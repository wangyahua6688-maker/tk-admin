...
func (bc *BizConfigController) ListHomePopups(c *gin.Context) {
	...
	// 构建列表查询，按排序升序、ID倒序返回。
	query := bc.db.Model(&models.WHomePopup{}).
...
func (bc *BizConfigController) UpdateHomePopup(c *gin.Context) {
	...
	// 执行更新并返回ID。
	if err := bc.db.Model(&models.WHomePopup{}).Where("id = ?", id).Updates(updates).Error; err != nil {
		utils.JSONError(c, http.StatusInternalServerError, err.Error())
		return
	}
...
func (bc *BizConfigController) DeleteHomePopup(c *gin.Context) {
	...
	// 执行删除。
	if err := bc.db.Delete(&models.WHomePopup{}, id).Error; err != nil {
		utils.JSONError(c, http.StatusInternalServerError, err.Error())
		return
	}
	utils.JSONOK(c, gin.H{"id": id})
