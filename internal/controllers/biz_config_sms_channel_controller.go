...
func (bc *BizConfigController) ListSMSChannels(c *gin.Context) {
	...
	// 构建查询并保证排序稳定。
	query := bc.db.Model(&models.WSMSChannel{}).Order("status DESC, id ASC").Limit(200)
...
func (bc *BizConfigController) UpdateSMSChannel(c *gin.Context) {
	...
	// 执行更新并返回。
	if err := bc.db.Model(&models.WSMSChannel{}).Where("id = ?", id).Updates(updates).Error; err != nil {
		utils.JSONError(c, http.StatusInternalServerError, err.Error())
		return
	}
...
func (bc *BizConfigController) DeleteSMSChannel(c *gin.Context) {
	...
	if err := bc.db.Delete(&models.WSMSChannel{}, id).Error; err != nil {
		utils.JSONError(c, http.StatusInternalServerError, err.Error())
		return
	}
	utils.JSONOK(c, gin.H{"id": id})
