...
func (bc *BizConfigController) UpdateSpecialLottery(c *gin.Context) {
	...
	if err := bc.db.Model(&models.WSpecialLottery{}).Where("id = ?", id).Updates(updates).Error; err != nil {
		utils.JSONError(c, 500, err.Error())
		return
	}
...
func (bc *BizConfigController) DeleteSpecialLottery(c *gin.Context) {
	...
	if err := bc.db.Delete(&models.WSpecialLottery{}, id).Error; err != nil {
		utils.JSONError(c, 500, err.Error())
		return
	}
	utils.JSONOK(c, gin.H{"id": id})
...
func (bc *BizConfigController) ListLotteryInfos(c *gin.Context) {
	var items []models.WLotteryInfo
	// 图库内容管理优先按更新时间倒序，避免运营修改后找不到记录。
	if err := bc.db.Order("updated_at DESC, id DESC").Limit(300).Find(&items).Error; err != nil {
		utils.JSONError(c, 500, err.Error())
		return
	}
	// 附带每条图纸的动物竞猜选项，供后台表单直接回显。
	optionNameMap, err := bc.queryOptionNameMap(items)
	if err != nil {
		utils.JSONError(c, 500, err.Error())
		return
	}
...
func (bc *BizConfigController) CreateLotteryInfo(c *gin.Context) {
	...
	categoryID, categoryTag, err := bc.resolveLotteryCategory(req.CategoryID, req.CategoryTag)
	if err != nil {
	...
	if err := bc.db.Transaction(func(tx *gorm.DB) error {
		...
		// 写入图纸动物竞猜选项（全量替换，避免脏数据残留）。
		return bc.replaceLotteryOptions(tx, item.ID, optionNames)
	}); err != nil {
		utils.JSONError(c, 500, err.Error())
		return
	}
...
func (bc *BizConfigController) UpdateLotteryInfo(c *gin.Context) {
	...
	var current models.WLotteryInfo
	...
	if req.CategoryID != nil || req.CategoryTag != nil {
		resolvedID, resolvedTag, resolveErr := bc.resolveLotteryCategory(
	...
	// 更新时同样维护“每彩种唯一当前期”约束。
	if err := bc.db.Transaction(func(tx *gorm.DB) error {
		if next.IsCurrent == 1 && next.SpecialLotteryID > 0 {
			if err := tx.Model(&models.WLotteryInfo{}).
		...
		if err := tx.Model(&models.WLotteryInfo{}).Where("id = ?", id).Updates(updates).Error; err != nil {
			return err
		}
		if updateOptions {
			return bc.replaceLotteryOptions(tx, id, optionNames)
		}
		return nil
	}); err != nil {
		utils.JSONError(c, 500, err.Error())
...
func (bc *BizConfigController) DeleteLotteryInfo(c *gin.Context) {
	...
	if err := bc.db.Delete(&models.WLotteryInfo{}, id).Error; err != nil {
		utils.JSONError(c, 500, err.Error())
		return
	}
	utils.JSONOK(c, gin.H{"id": id})
...
// resolveLotteryCategory 将分类输入（category_id 或 category_tag）解析为标准分类。
func (bc *BizConfigController) resolveLotteryCategory(categoryID *uint, categoryTag *string) (uint, string, error) {
	// 优先按 category_id 解析，确保后台强关联分类配置表。
	if categoryID != nil && *categoryID > 0 {
		var cat models.WLotteryCategory
		if err := bc.db.First(&cat, *categoryID).Error; err != nil {
			return 0, "", fmt.Errorf("category_id not found")
		}
		return cat.ID, strings.TrimSpace(cat.CategoryKey), nil
	}

	// 兼容旧请求：仅传 category_tag 时，尝试按 key/name 反查分类。
	tag := strings.TrimSpace(safeString(categoryTag))
	if tag != "" {
		var cat models.WLotteryCategory
		if err := bc.db.Where("category_key = ? OR name = ?", tag, tag).Order("id ASC").First(&cat).Error; err == nil {
			return cat.ID, strings.TrimSpace(cat.CategoryKey), nil
		}
	}
	return 0, "", fmt.Errorf("category_id required")
}
...
// queryOptionNameMap 批量查询图纸动物竞猜选项并按图纸 ID 分组。
func (bc *BizConfigController) queryOptionNameMap(items []models.WLotteryInfo) (map[uint][]string, error) {
	...
	var options []models.WLotteryOption
	if err := bc.db.Where("lottery_info_id IN ?", infoIDs).Order("sort ASC, id ASC").Find(&options).Error; err != nil {
		return nil, err
	}
	for _, opt := range options {
		...
		result[opt.LotteryInfoID] = append(result[opt.LotteryInfoID], name)
	}
	return result, nil
}
...
// replaceLotteryOptions 全量替换图纸的动物竞猜选项，默认保留同名历史票数。
func (bc *BizConfigController) replaceLotteryOptions(tx *gorm.DB, infoID uint, optionNames []string) error {
	// 1) 读取旧选项票数，保证“编辑选项顺序”不导致票数丢失。
